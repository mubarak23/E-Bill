...
pub trait BillStoreApi: Send + Sync {
    ...
    async fn write_bill_to_file(&self, bill_id: &str, bytes: &[u8]) -> Result<()>;
    ...
    async fn read_bill_chain_from_file(&self, bill_id: &str) -> Result<Chain>;
    ...
    async fn open_attached_file(&self, bill_id: &str, file_name: &str) -> Result<Vec<u8>>;
    ...
    async fn write_bill_keys_to_file_as_bytes(&self, bill_id: &str, bytes: &[u8]) -> Result<()>;
...
impl BillStoreApi for FileBasedBillStore {
    ...
    async fn save_attached_file(
        ...
        let dest_dir = Path::new(&self.files_folder).join(bill_id);
        if !dest_dir.exists() {
    ...
    async fn open_attached_file(&self, bill_id: &str, file_name: &str) -> Result<Vec<u8>> {
        let path = Path::new(&self.files_folder).join(bill_id).join(file_name);
